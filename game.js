...
function spawnEnemy() {
  if (Date.now() - lastEnemySpawn > ENEMY_SPAWN_INTERVAL) {
    ...
    enemies.push({ x: canvas.width, y: Math.random() * (canvas.height - size), width: size, height: size, color: '#00ff7f' });
...
function handlePowerUpActivation() {
  if (keys.Shift && !shiftPressed) {
    player.activePowerUps = powerUpState.activate(player.activePowerUps);
    shiftPressed = true;
  } else if (!keys.Shift) {
    shiftPressed = false;
  }
...
function drawPowerUpCapsules() {
  powerUpCapsules.forEach((capsule, index) => {
...
function drawPowerUpMeter() {
  ...
  meter.forEach((name, index) => {
    ...
    let isActive = false;
    if (name === 'SPEED UP' && player.activePowerUps.speedLevel > 0) {
      isActive = true;
      displayName = `SPEED ${player.activePowerUps.speedLevel}`;
    }
...
function gameLoop() {
  ...
  if (keys[' ']) {
    if (player.activePowerUps.shotType === 'laser') { fireLaser(); } else { fireNormalAndDouble(); }
  }
  ...
  powerUpCapsules.forEach((c, i) => { c.x -= ENEMY_SPEED; if (c.x < 0) powerUpCapsules.splice(i, 1); });
